...
/// <summary>
/// Mirrors <see cref="GrpcTemplateGenerationTests"/>, scoped to the worker MVP
/// (sqlite + EF Core + Aspire, no provider/orm/orchestrator/trigger symbols).
/// </summary>
[Trait("Category", "Integration")]
public class WorkerTemplateGenerationTests
{
    private const string LocalNuGetFeedEnvironmentVariableName = "DORN_LOCAL_NUGET_FEED";

    [Fact]
    public async Task GenerateAndBuild_DornWorkerTemplate_ProducesBuildableSolution()
    {
        ...
        var outputDirectory = Path.Combine(
            Path.GetTempPath(),
            $"dorn-tests-worker-{Guid.NewGuid():N}"
        );
        try
        {
            var request = new GenerationRequest(
                "dorn-worker",
                "DornIntegrationTestWorkerApp",
                outputDirectory
            );
            ...
            Assert.Equal("DornIntegrationTestWorkerApp.slnx", Path.GetFileName(slnFiles[0]));
