...
| Local dev/CI feed (`build-test.yml`) | A **local-only tag** (`git tag -f "v<pinned>"`, never pushed) is created on HEAD immediately before each `dotnet pack`, matching the version already pinned in the checked-in `templates/*/Directory.Packages.props`, then deleted; `-f` handles the case where that version string coincides with a real historical release tag already in the checkout |
