...
public sealed class NewGrpcCommand(
    IGenerationEngine generationEngine,
    IProcessRunner processRunner,
    IAnsiConsole console,
    IDornTheme theme
) : AsyncCommand<NewGrpcSettings>
{
    private const string TemplateShortName = "dorn-grpc";
    ...
    protected override Task<int> ExecuteAsync(
        ...
        NewGrpcSettings settings,
        CancellationToken cancellationToken
    ) => RunAsync(settings, cancellationToken);

    /// <summary>
    /// Runs the new grpc command logic. Public so unit tests can drive the command
    /// directly without going through the Spectre.Console.Cli command pipeline.
    /// </summary>
    public async Task<int> RunAsync(NewGrpcSettings settings, CancellationToken cancellationToken)
    {
